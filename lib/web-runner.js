...
exports.run = function(list, options) {
  ...
  try {
    // TODO: this seems a strange way to do this, fix it
  ...
  function checkQueue() {
    ...
    var opts =
      ...
      , onSuiteCompleted: function(results) {
          workerHandlers.onSuiteCompleted(suite, results);
        }
      , onSuiteError: function(error, tests) {
          workerHandlers.onSuiteError(suite, error, tests);
        }
  ...
  var workerHandlers =
    ...
    , onSuiteCompleted: function(suite, results) {
        var msg = {cmd: 'suiteDone', suite: suite.index, numSuccesses: results.numSuccesses, numFailures: results.numFailures};
        socket.broadcast(JSON.stringify(msg));

        cleanupSuite(suite);
      }
    , onSuiteRequireError: function(suite, obj) {
        socket.broadcast(JSON.stringify(obj));

        cleanupSuite(suite);
      }
    , onSuiteError: function(suite, error, tests) {
        var msg = {cmd: 'suiteError', suite: suite.index, error: error, tests: tests};
        socket.broadcast(JSON.stringify(msg));
        cleanupSuite(suite);
      }
    , onSuiteExit: function(suite, obj) {
        socket.broadcast(JSON.stringify(obj));
        cleanupSuite(suite);
      }
    }
