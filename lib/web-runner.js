...
exports.run = function(list, options, args, callback) {
  ...
  asynctesting.loadSuites(list, options.suiteName, startServer);
  ...
  function startServer(loaded) {
    ...
    socket.on('connection', function(client){
      ...
      var msg = [];
      for (var i = 0; i < suites.length; i++) {
        msg.push(createSuiteMessage(suites[i]));
      }
      client.send(JSON.stringify({cmd: 'suitesList', suites: msg}));
  ...
  function createSuiteMessage(item) {
    var msg = {};
    msg.name = item.name;
    msg.index = item.index;
    msg.tests = {};
    for (var name in item.suite) {
      msg.tests[name] = { func: item.suite[name] + '' };
    }
    msg.parallel = item.parallel;
  ...
  function checkQueue() {
    ...
    console.log(module);
    console.log(module.moduleCache);
    ...
    if (!worker) {
      worker = new Worker(path.join(__dirname, 'web-runner-worker.js'));
