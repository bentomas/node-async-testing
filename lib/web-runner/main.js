...
(function() {
  ...
  var handlers =
    ...
    , suitesList: function(obj) {
        while(suitesList.firstChild) {
          suitesList.removeChild(suitesList.firstChild);
        }

        suites = obj.suites;

        suites.forEach(buildSuiteElement);
      }
    , suiteStart: function(obj) {
        ...
        var nonSpecific = el.getElementsByClassName('non-specific-errors');
        while(nonSpecific.length) {
          nonSpecific[0].parentNode.removeChild(nonSpecific[0]);
        }
      }
  ...
  function clearSuiteResults(suite) {
    ...
    var els = suite.el.getElementsByClassName('done');
    while(els.length) {
  ...
  function buildSuiteElement(suite) {
    ...
    suitesList.appendChild(suiteLi);
  }
